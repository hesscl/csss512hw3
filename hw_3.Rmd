...
house <- groupedData(DemHouseShare ~ PartisanMidterm + PresUnem + GovUnem + PresCoattails + GovCoattails + (Year * Midwest) + (Year * South) + (Year * West) | State, data = house)
...
house <- house %>%
  ...
  mutate(time = row_number(),
         timeXmidwest = time * Midwest,
         timeXwest = time * West,
         timeXsouth = time * South)
house_p <- pdata.frame(house, index = "State")

fe_m2_b <- plm(DemHouseShare ~ PartisanMidterm + PresUnem + GovUnem + PresCoattails + GovCoattails + time + timeXmidwest + timeXwest + timeXsouth + DemHouseShareL1, data= house_p, model = "within", effect = "individual")
summary(fe_m2_b)
...
simbetas <- cbind(rep(mean(fe.fe2_b), sims), simparam[,2:ncol(simparam)])

formula <- "DemHouseShare ~ PartisanMidterm + PresUnem + GovUnem + PresCoattails + GovCoattails + time + timeXmidwest + ytimeXwest + timeXsouth"  
formula <- as.formula(formula)
  
periods.out <- 2
xhyp <- cfMake(formula, house_p, periods.out)
for(i in 1:periods.out){
  xhyp <- cfChange(xhyp, "Year", x=2019, scen=1)
  xhyp <- cfChange(xhyp, "Year", x=2021, scen=2)

  xhyp <- cfChange(xhyp, "PartisanMidterm", x = -1, xpre = 0, scen=1)
  xhyp <- cfChange(xhyp, "PartisanMidterm", x = 0, scen=2)
  
  xhyp <- cfChange(xhyp, "PresUnem", x=-1*(3.6-5.97), scen=i)
  xhyp <- cfChange(xhyp, "GovUnem", x=-1*(3.6-5.97), scen=1)
  xhyp <- cfChange(xhyp, "GovUnem", x=1*(3.6-5.97), scen=i)
...
phi <- mean(simphi) 
lagY <- mean(house$DemHouseShareL1, na.rm=T) # Hypothetical previous change in Y for simulation
